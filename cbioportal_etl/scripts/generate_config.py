...
def get_cases_info(db_cur, study, tbl_name):
    """
    ...
    tumor_model_studies = {
        "bllnos_sd_z6mwd3h0_2018", "chdm_phs001643_2018", "chdm_phs002301_2021", 
        "chdm_sd_7spqtt8m", "open_chordoma", "os_sd_zxjffmef_2015"
    }
    ...
    case_list_name_3way = "Tumor and model samples with mutation, CNA and mRNA data" if is_tumor_model else "Tumor samples with mutation, CNA and mRNA data"
    # cases_cna
    case_list_desc_cna = f"{case_list_desc} with CNA data"
    case_list_name_cna = "Tumor and model samples with CNA data" if is_tumor_model else "Tumor Samples with CNA data"
    # cases_cnaseq
    case_list_desc_cnaseq = "All tumor samples with mutation and CNA data"
    case_list_name_cnaseq = "Tumor samples with mutation and CNA data" if is_tumor_model else "Tumor samples with mutation and CNA data"
    # cases_RNA_Seq_v2_mRNA
    case_list_desc_rna = "All samples with mRNA expression data"
    case_list_name_rna = "Tumor and model samples with mRNA data (RNA Seq V2)" if is_tumor_model else "Tumor Samples with mRNA data (RNA Seq V2)"
    # cases_sequenced
    case_list_desc_sequenced = f"{case_list_desc} with mutation data"
    case_list_name_sequenced = "Tumor and model samples with mutations" if is_tumor_model else "Tumor samples with mutations"
    # cases_sv
    case_list_desc_sv = f"{case_list_desc} with fusion data"
    case_list_name_sv = "Tumor and model samples with fusions" if is_tumor_model else "Tumor samples with fusions"
...
def get_database_pulls(db_cur, study, tbl_name):
    """
    Queries DWH for study-specific timeline tables.
    ...
    for key, info in timeline_tables.items():
        check_table_sql = sql.SQL("SELECT to_regclass('prod_cbio.{}') IS NOT NULL;").format(sql.Identifier(info["table"]))
        ...
        if table_exists:
            ...
            entries.append([
                study, "data_sheets", original_config, correct_subkey, 
                json.dumps({
...
def generate_json(study_df, study):
    result = {}
...
def generate_tsv(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        cases_df = get_cases_info(cur, study, tbl_name)
        database_pulls_df = get_database_pulls(cur, study, tbl_name)
...
def main():
    ...
    parser.add_argument("-ctsv", "--config-tsv", action="store", dest="config_tsv", help="Path to the input TSV file containing static values (preset in repo)")
    parser.add_argument("-stsv", "--study-tsv", action="store", dest="study_tsv", help="Path to the input TSV file containing completed values for json generation")

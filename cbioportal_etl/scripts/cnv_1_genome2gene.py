#!/usr/bin/env python3
"""Convert ControlFreeC results to per gene, filtered copy number."""
import argparse
...
def process_cnv(cpath: str) -> None:
    ...
    try:
        if cpath != "":
            ...
            with (
                open(out_dir + temp_filtered_fname, "w") as temp_filtered,
                open(cpath) as in_cnv,
            ):
                head = next(in_cnv)
                header = head.rstrip("\n").split("\t")
                ...
                for cnv in in_cnv:
                    ...
                    if cnv_data[wilcox_idx] == "NA" or cnv_data[ks_idx] == "NA":
                        ...
                        float(cnv_data[wilcox_idx]) < PVALUE
                        and float(cnv_data[ks_idx]) < PVALUE
                    ):
            ...
            out_gene_cnv_only = f"perl {cnv_cp_only} {hugo_tsv} {out_dir}{temp_genes_fname} > {out_dir}{temp_genes_fname}.copy_number"

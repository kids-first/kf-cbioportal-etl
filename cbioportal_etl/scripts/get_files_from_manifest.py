...
def download_aws(file_type, key_dict, overwrite, err_types):
    """
...
def download_sbg(file_type, selected, aws_tbl, api, overwrite, err_types):
    """
...
def mt_type_download(file_type, key_dict, selected, api, overwrite, err_types, sbg_profile, aws_tbl):
    """
...
def run_py(args):
    ...
    sys.stderr.flush()
    selected = manifest_concat[manifest_concat["file_type"].isin(file_types_list)]
    ...
    with concurrent.futures.ThreadPoolExecutor(16) as executor:
        results = {executor.submit(mt_type_download, ftype, key_dict, selected, api, args.overwrite, err_types, args.sbg_profile, args.aws_tbl): ftype for ftype in file_types_list}
...
def main():    
    ...
    parser.add_argument("-m", "--manifest", action="store", dest="manifest", help="csv list of of genomic file location manifests")
    parser.add_argument( "-f", "--file-types", action="store", dest="fts", help="csv list of workflow types to download")
    parser.add_argument("-at", "--aws-tbl", action="store", dest="aws_tbl", help="Table with bucket name and keys to subset on")
    parser.add_argument("-sp", "--sbg-profile", action="store", dest="sbg_profile", help="sbg profile name. Leave blank if using AWS instead")
    parser.add_argument("-c", "--cbio", action="store", dest="cbio", help="Add cbio manifest to limit downloads")
    parser.add_argument("-ao", "--active-only", default=False, action="store_true", dest="active_only", help="Set to grab only active files. Recommended.")
    parser.add_argument("-rm", "--rm-na", default=False, action="store_true", dest="rm_na", help="Remove entries where file_id and s3_path are NA. Useful for studies (like pbta_all) with external files not to be downloaded when using cbio_file_name input file as manifest")
    parser.add_argument("-d", "--debug", default=False, action="store_true", dest="debug", help="Just output manifest subset to see what would be grabbed")
    parser.add_argument("-o", "--overwrite", default=False, action="store_true", dest="overwrite", help="If set, overwrite if file exists")

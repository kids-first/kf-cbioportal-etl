...
def download_aws(file_type, key_dict, overwrite, err_types):
    ...
    for key in key_dict:
        ...
        print(
            "Grabbing {} files using key {}".format(len(sub_file_list), key),
            file=sys.stderr,
        )
        for loc in sub_file_list:
            ...
            if overwrite or not os.path.isfile(out):
                ...
                try:
                    ...
                    print(
                        "{} could not download from {} using key {}".format(
                            e, loc, key
                        ),
                        file=sys.stderr,
                    )
                ...
                sys.stderr.write(
                    "Skipping " + out + " it exists and overwrite not set\n"
                )
...
def download_sbg(file_type, selected, aws_tbl, api, overwrite, err_types):
    ...
    for loc in sub_file_list:
        try:
            ...
            print(
                "Failed to get file with id "
                + loc
                + ". Will try once more in 3 seconds",
                file=sys.stderr,
            )
        ...
        if overwrite or not os.path.isfile(out):
            ...
            print(
                "Skipping " + out + " it exists and overwrite not set", file=sys.stderr
            )
    return 0


def mt_type_download(
    file_type: str, key_dict: dict[str, dict[str, Any]], selected: pd.DataFrame, api: sbg.Api | None, overwrite: bool, err_types: dict[str, int], sbg_profile: str, aws_tbl: str
) -> None:
    """Download files from each desired file type at the same time.

    Picks the download protocol based on provided args
    Args:
        file_type: String reresentation of genomic file type from ETL file
    ...
    if len(selected.loc[selected["file_type"] == file_type]):
        ...
        print(
            "WARN: No files of type "
            + file_type
            + " in which file_id and s3_path is not NA. Skipping!",
            file=sys.stderr,
        )
    sys.stderr.flush()
...
def run_py(args: argparse.Namespace) -> int:
        ...
        with open(args.aws_tbl) as kl:
            for line in kl:
                ...
                if key not in key_dict:
                    key_dict[key] = {}
                    key_dict[key]["manifest"] = selected[
                        selected["s3_path"].str.contains(bucket)
                    ]
                ...
                try:
                    ...
                    print(e, file=sys.stderr)
                    print(
                        f"Bucket access ERROR: {bucket}\t{key}",
                        file=sys.stderr,
                    )
        if bucket_errs:
           sys. exit(1)
    if args.sbg_profile is not None:
        ...
        api = sbg.Api(
            config=config, error_handlers=[rate_limit_sleeper, maintenance_sleeper]
        )
    if not check:
    ...
    for protocol, count in err_types.items():
        if count:
            flag += count
            print(
                f"ERROR: {protocol} {count} failure(s) occurred",
                file=sys.stderr,
            )
    if flag:
        sys.exit(1)
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        dest="rm_na",
        help="Remove entries where file_id and s3_path are NA. Useful for studies (like pbta_all) with external files not to be downloaded when using cbio_file_name input file as manifest",
    )
    parser.add_argument(
        "-d",
        "--debug",

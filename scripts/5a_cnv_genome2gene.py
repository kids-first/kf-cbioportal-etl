...
def process_cnv(cpath):
    try:
        if cpath != '':
            ...
            out_gene_cnv_only = 'perl ' + cnv_cp_only + ' ' + hugo_tsv + ' ' + out_dir + temp + ' > ' + out_dir + temp + '.copy_number'
...
parser = argparse.ArgumentParser(description='Convert controlFreeC cnv genome coords to gene level')

...
def add_fusion_file(config_data, study_dir, study_resources):
    try:
        ...
        fusion_data = fusion_data.groupby(['Gene1A', 'Tumor_Sample_Barcode', 'FusionName', 'Fusion_Type', 'Center'])[
            'Caller'].unique().apply(', '.join).reset_index()
        fusion_data['Caller'] = fusion_data['Caller'].str.upper()
        fusion_data.rename(
            columns={"Gene1A": "Hugo_Symbol", "FusionName": "Fusion", "Caller": "Method", "Fusion_Type": "Frame"},
            inplace=True)
        ...
        # Reorder table
        order_list = ['Hugo_Symbol', 'Entrez_Gene_Id', 'Center', 'Tumor_Sample_Barcode', 'Fusion', 'DNA_support',
                      'RNA_support', 'Method', 'Frame']

...
psql -h d3b-warehouse-aurora-prd.d3b.io -U brownm28 postgres -c "SET search_path TO welshm3_dev_schema; COPY(select * from genomics_file_manifest where file_type in ('DGD_MAF', 'DGD_FUSION')) TO STDOUT WITH CSV HEADER DELIMITER E'\t'" > dgd_genomics_file_manifest.txt
...
psql -h d3b-warehouse-aurora-prd.d3b.io -U brownm28 postgres -c "SET search_path TO prod_cbio; COPY(select * from cbio_sq_info_etl_resource) TO STDOUT WITH CSV HEADER DELIMITER E'\t'" > seq_center_resource.txt
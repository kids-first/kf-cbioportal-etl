...
psql -h d3b-warehouse-aurora-prd.d3b.io -U brownm28 postgres -c "SET search_path TO bix_workflows; COPY(select * from pbta_all_cbio_genomics_file_etl_dict) TO STDOUT WITH CSV HEADER DELIMITER E'\t'" > cbio_file_name_id.txt
...
# Data gene matrix
psql -h d3b-warehouse-aurora-prd.d3b.io -U brownm28 postgres -c "SET search_path TO bix_workflows; COPY(select * from pbta_all_data_gene_matrix) TO STDOUT WITH CSV HEADER DELIMITER E'\t'" > datasheets/data_gene_matrix_CHOP.txt

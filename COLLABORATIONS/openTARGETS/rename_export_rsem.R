...
option_list <- list(
  ...
  make_option(
    opt_str = "--map_id",
    type = "character",
    help = "mapping ID file with headers: Cbio_project	T_CL_BS_ID	Sample Type	Cbio_Tumor_Name	File_Type"
  )
)
#parse options
...
subset_rna = rna[,which(colnames(rna) %in% map_ids$T_CL_BS_ID)]
rownames(subset_rna) = rownames(rna)
# Rename BS IDs to cBio IDs
message("Renaming samples and outputting tpm file")
setnames(subset_rna, old=as.character(map_ids$T_CL_BS_ID), new=as.character(map_ids$Cbio_Tumor_Name))

...
def query_dataservice_bs_id(url, bs_id, bs_attrs, pt_attrs, dx_attrs, outcome_attrs):
    ...
    # dx can sometimes have multiple values, so dixt is used to store them all
    dx_dict = {}
    ...
    result.append(process_attr_dict(bs_attrs, bs_info))
    result.append(process_attr_dict(pt_attrs, pt_info))
...
parser.add_argument('-u', '--kf-url', action='store', dest='url',
                    help='Kids First data service url', const='https://kf-api-dataservice.kidsfirstdrc.org/')
